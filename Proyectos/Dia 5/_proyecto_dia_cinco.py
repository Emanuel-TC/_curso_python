...
def seleccionar_palabras(lista):
    ...
    palabra = [] #definimos palabra como una lista para después tener mejor control de las letras de la palabra
...
def espacios_vacios(palabra):
    palabra_vacia = [] #se crea una lista vacía para almacenar un guión por cada letra de la palabra escogida
    for letra in palabra:
        palabra_vacia.append("_")
    return palabra_vacia #regresa la lista con guíones en lugar de las letras de la palabra
...
def revisar_espacios(palabra):
    if "_" in palabra: # si hay espacios vacíos devolvemos True
        return True
...
palabra_escogida = seleccionar_palabras(lista_palabras) #Guardamos en una variable la palabra seleccionada
palabra_sin_letras = espacios_vacios(palabra_escogida) #Guardamos en una variable la lista que se llenó con guíones
...
while vidas < 7: #mientras no se haya equivocado más de 6 veces
    ...
    if revisar_letra_en_palabra(letra,palabra_escogida) == False: #revisamos si la letra está o no en la palabra, si no no está, entonces
        vidas += 1 #el contador de vidas aumenta
        if vidas >= 7: #y si sus vidas sobrepasaron el límite,
            print(f"Lo siento, has fallado, la palabra era {palabra_escogida}") #imprime que ha fallado, y le muestra la palabra que debía encontrar
        ...
        letra_encontrada = revisar_letra_en_palabra(letra, palabra_escogida) #almacena en una variable la letra que encontró
        palabra_llenada = reemplaza_espacio_por_letra(letra_encontrada, palabra_escogida) #llena una nueva lista con la letra(s) encontrada en el lugar que debe ir
        ...
        if revisar_espacios(palabra_llenada) == False: #finalmente revisa si todavía hay espacios vacíos (guíones)
            print(f"Felicidades, adivinaste, la palabra escogida si es: {palabra_escogida}") #si ya no hay guíones #lo felicita e imprime nuevamente la palabra que encontró
